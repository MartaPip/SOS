...
def check_arrival_time_fast(jobs,last_check_time,current_time):
    #TO DO: don't need the loop, could just order by release time and move with index
    new_arrivals = []
    # at time 0 jusy append jobs arrived at that time
    if current_time==0: 
        for idx, job in enumerate(jobs):
...
def assigment(RANDOM_assigment,job,m,machines):
    ...
    if RANDOM_assigment: machine=randint(0,m-1)
        ...
        machine=np.min(np.where(machines==firts_free)) #assign to machine that is fired before
    ...
    else: #machine is free berore alpha-point--> need to wait for job to arrive
...
def run_one(jobs, m,RANDOM_alpha,RANDOM_assigment,alpha,time_step):
    #np.random.seed(10), done in main
...
def run_one_both_fast(jobs, m,alpha):
    ...
    # Note: Use simulation knowladge if arrival times to limit the number of iterations by picking a varying time step. The end results will be the same but the computing time will be higly reduced.
    #The knowladge of the arrival times is ONLY used for the time_step and not for making any decisiom in the order of job processing
    ...
    # Until all the jobs are not processed in the single fast machine
    while sum(job.done_1 for job in jobs)<len(jobs):
        time_step=Arrivals[arrival]-Arrivals[arrival-1]#=future time a job arrive-current time
        arrival=arrival+1
        ite=ite+1
        #if ite%50==0: print("iteration",ite)
    ...
    #print(sum( (job.MBT-(job.CV-1)*job.expected/2) for job in jobs))
